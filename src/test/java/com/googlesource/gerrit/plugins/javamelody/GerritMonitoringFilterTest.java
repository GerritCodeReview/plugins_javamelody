...
public class GerritMonitoringFilterTest {
  private static final Pattern GROUPING_PATTERN =
      Pattern.compile(GerritMonitoringFilter.JavamelodyFilter.GERRIT_GROUPING);
  ...
  public void testGroupingPattern() throws Exception {
    ...
    String result =
        GROUPING_PATTERN
            .matcher(
                "/changes/?q=test_repo~refs/meta/config~Iee4ae093988f8e2a1ad6d413450eec48b67124a2&n=")
            .replaceAll(replacement);
    assertThat(result).named("Change ID triplet grouping").isEqualTo("/changes/?q=$&n=");
    ...
    result = GROUPING_PATTERN.matcher("/files/%2FCOMMIT_MSG/reviewed").replaceAll(replacement);
    assertThat(result).named("Grouping by file").isEqualTo("/files/$reviewed");

    result =
        GROUPING_PATTERN
            .matcher("/projects/test_repo/lfs~lfs:config-project")
            .replaceAll(replacement);
    assertThat(result).named("Grouping by projects").isEqualTo("/projects/$lfs~lfs:config-project");

    result = GROUPING_PATTERN.matcher("/accounts/self/avatar.change.url").replaceAll(replacement);
    assertThat(result).named("Grouping by account").isEqualTo("/accounts/$avatar.change.url");

    result = GROUPING_PATTERN.matcher("/test_repo/git-upload-pack").replaceAll(replacement);
    assertThat(result).named("Grouping git-upload-pack").isEqualTo("$/git-upload-pack");

    result = GROUPING_PATTERN.matcher("/test_repo/git-receive-pack").replaceAll(replacement);
    assertThat(result).named("Grouping git-receive-pack").isEqualTo("$/git-receive-pack");

    result = GROUPING_PATTERN.matcher("/test_repo.git/info/lfs/locks").replaceAll(replacement);
    assertThat(result).named("Grouping Git and LFS operations").isEqualTo("$/info/lfs/locks");

    result = GROUPING_PATTERN.matcher("/changes/30/revisions/1/commit").replaceAll(replacement);
    assertThat(result).named("Grouping numbers").isEqualTo("/changes/$/revisions/$/commit");

    // grouping multiple patters in one input
    result =
        GROUPING_PATTERN
            .matcher(
                "/changes/30/revisions/05810c7a315ba6c52c150c237d84d05b2ebc3086/files/%2FCOMMIT_MSG/diff")
            .replaceAll(replacement);
    assertThat(result)
