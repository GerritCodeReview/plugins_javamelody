...
public class GerritMonitoringFilterTest {
  ...
  public void testGroupingPattern() throws Exception {
    ...
    String result =
        GROUPING_PATTERN
            .matcher(
                "/changes/?q=test_repo~refs/meta/config~Iee4ae093988f8e2a1ad6d413450eec48b67124a2")
            .replaceAll(dollar);
    assertThat(result).named("Change ID triplet grouping").isEqualTo("/changes/?q=$");
    ...
    result = GROUPING_PATTERN.matcher("/files/%2FCOMMIT_MSG/reviewed").replaceAll(dollar);
    assertThat(result).named("Grouping by file").isEqualTo("/files/$reviewed");

    result =
        GROUPING_PATTERN.matcher("/projects/test_repo/lfs~lfs:config-project").replaceAll(dollar);
